...
@section('content')
    ...
    <section class="section">
        ...
        <div class="row mb-4">
            ...
            @can('dashboard_buy_sell_section')<div class="col-md-3 mb-3"><div class="dashboard-stat"><small>Net active TTB</small><strong>{{ number_format(abs($totalRunningActiveTTB), 2) }} {{ $totalRunningActiveTTB > 0 ? 'Buy' : ($totalRunningActiveTTB < 0 ? 'Sell' : '') }}</strong><div class="sub">Buy {{ number_format($totalRunningBuyTTB, 2) }} · Sell {{ number_format($totalRunningSellTTB, 2) }}</div></div></div>@endcan
            @can('dashboard_transection_section')<div class="col-md-3 mb-3"><div class="dashboard-stat"><small>Total cash movement</small><strong>AED {{ number_format($totalTransactionAmount, 2) }}</strong><div class="sub">Deposits + withdrawals</div></div></div><div class="col-md-3 mb-3"><div class="dashboard-stat"><small>Deposits / withdrawals</small><strong>AED {{ number_format($totalDepositAmount, 2) }}</strong><div class="sub">Withdrawals AED {{ number_format($totalWithDrawAmount, 2) }}</div></div></div>@endcan
        </div>
